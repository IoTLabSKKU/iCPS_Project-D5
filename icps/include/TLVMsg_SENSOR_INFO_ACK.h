//description: SENSOR_INFO_ACK, (LAPTOP -> SMARTPHONE/OBU/ALIX)

#ifndef TLVMsg_SENSOR_INFO_ACK_H_
#define TLVMsg_SENSOR_INFO_ACK_H_
#include <vector>
#include <stdint.h>
#include <sstream>
#include "TLVHelper.h"
...
class TLVMsg_SENSOR_INFO_ACK{
public:
	static const TLVType type = TLVType_SENSOR_INFO_ACK;
	int length;

	TLVClass_RESPONSE_CODE m_RESPONSE_CODE;

public:
	TLVMsg_SENSOR_INFO_ACK(){length = -1;}
	...
	string toString(int tabIndex=0){
		...
		ss<<setfill('0')<<setw(6)<<hex<<uppercase<<type<<" "<<setfill('0')<<setw(6)<<hex<<uppercase<<length<<" (SENSOR_INFO_ACK, "<<dec<<nouppercase<<length<<")"<<endl;
		ss << header;
