//description: REMOTE_INFO_ACK, (LAPTOP -> OBU/ALIX)

#ifndef TLVMsg_REMOTE_INFO_ACK_H_
#define TLVMsg_REMOTE_INFO_ACK_H_
#include <vector>
#include <stdint.h>
#include <sstream>
#include "TLVHelper.h"
...
class TLVMsg_REMOTE_INFO_ACK{
public:
	static const TLVType type = TLVType_REMOTE_INFO_ACK;
	int length;

	TLVClass_RESPONSE_CODE m_RESPONSE_CODE;

public:
	TLVMsg_REMOTE_INFO_ACK(){length = -1;}
	...
	string toString(int tabIndex=0){
		...
		ss<<setfill('0')<<setw(6)<<hex<<uppercase<<type<<" "<<setfill('0')<<setw(6)<<hex<<uppercase<<length<<" (REMOTE_INFO_ACK, "<<dec<<nouppercase<<length<<")"<<endl;
		ss << header;
