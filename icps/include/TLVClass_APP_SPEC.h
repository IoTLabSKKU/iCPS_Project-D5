...
#ifndef TLVClass_APP_SPEC_H_
#define TLVClass_APP_SPEC_H_
#include <vector>
#include <stdint.h>
#include <sstream>
#include "TLVHelper.h"
...
class TLVClass_APP_SPEC{
public:
	static const TLVType type = TLVType_APP_SPEC;
...
public:
	...
	string toString(int tabIndex=0){
		...
		ss<<setfill('0')<<setw(6)<<hex<<uppercase<<type<<" "<<setfill('0')<<setw(6)<<hex<<uppercase<<length<<" (APP_SPEC, "<<dec<<nouppercase<<length<<")"<<endl;
		ss << header;
