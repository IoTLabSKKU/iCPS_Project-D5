//description: MOBILITY_SUMMARY_ACK, (SMARTPHONE -> LAPTOP)

#ifndef TLVMsg_MOBILITY_SUMMARY_ACK_H_
#define TLVMsg_MOBILITY_SUMMARY_ACK_H_
#include <vector>
#include <stdint.h>
#include <sstream>
#include "TLVHelper.h"
...
class TLVMsg_MOBILITY_SUMMARY_ACK{
public:
	static const TLVType type = TLVType_MOBILITY_SUMMARY_ACK;
	int length;

	TLVClass_RESPONSE_CODE m_RESPONSE_CODE;

public:
	TLVMsg_MOBILITY_SUMMARY_ACK(){length = -1;}
	...
	string toString(int tabIndex=0){
		...
		ss<<setfill('0')<<setw(6)<<hex<<uppercase<<type<<" "<<setfill('0')<<setw(6)<<hex<<uppercase<<length<<" (MOBILITY_SUMMARY_ACK, "<<dec<<nouppercase<<length<<")"<<endl;
		ss << header;
