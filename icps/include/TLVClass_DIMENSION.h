...
#ifndef TLVClass_DIMENSION_H_
#define TLVClass_DIMENSION_H_
#include <vector>
#include <stdint.h>
#include <sstream>
#include "TLVHelper.h"
...
class TLVClass_DIMENSION{
public:
	static const TLVType type = TLVType_DIMENSION;
...
public:
	...
	string toString(int tabIndex=0){
		...
		ss<<setfill('0')<<setw(6)<<hex<<uppercase<<type<<" "<<setfill('0')<<setw(6)<<hex<<uppercase<<length<<" (DIMENSION, "<<dec<<nouppercase<<length<<")"<<endl;
		ss << header;
