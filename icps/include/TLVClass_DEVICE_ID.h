...
#ifndef TLVClass_DEVICE_ID_H_
#define TLVClass_DEVICE_ID_H_
#include <vector>
#include <stdint.h>
#include <sstream>
#include "TLVHelper.h"
...
class TLVClass_DEVICE_ID{
public:
	static const TLVType type = TLVType_DEVICE_ID;
...
public:
	...
	string toString(int tabIndex=0){
		...
		ss<<setfill('0')<<setw(6)<<hex<<uppercase<<type<<" "<<setfill('0')<<setw(6)<<hex<<uppercase<<length<<" (DEVICE_ID, "<<dec<<nouppercase<<length<<")"<<endl;
		ss << header;
