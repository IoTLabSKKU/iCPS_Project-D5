...
#ifndef TLVClass_RESPONSE_CODE_H_
#define TLVClass_RESPONSE_CODE_H_
#include <vector>
#include <stdint.h>
#include <sstream>
#include "TLVHelper.h"
...
class TLVClass_RESPONSE_CODE{
public:
	static const TLVType type = TLVType_RESPONSE_CODE;
...
public:
	...
	string toString(int tabIndex=0){
		...
		ss<<setfill('0')<<setw(6)<<hex<<uppercase<<type<<" "<<setfill('0')<<setw(6)<<hex<<uppercase<<length<<" (RESPONSE_CODE, "<<dec<<nouppercase<<length<<")"<<endl;
		ss << header;
