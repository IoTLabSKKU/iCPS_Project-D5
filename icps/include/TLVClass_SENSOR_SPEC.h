//description: SENSOR_SPEC, 

#ifndef TLVClass_SENSOR_SPEC_H_
#define TLVClass_SENSOR_SPEC_H_
#include <vector>
#include <stdint.h>
#include <sstream>
#include "TLVHelper.h"
...
class TLVClass_SENSOR_SPEC{
public:
	static const TLVType type = TLVType_SENSOR_SPEC;
...
public:
	...
	string toString(int tabIndex=0){
		...
		ss<<setfill('0')<<setw(6)<<hex<<uppercase<<type<<" "<<setfill('0')<<setw(6)<<hex<<uppercase<<length<<" (SENSOR_SPEC, "<<dec<<nouppercase<<length<<")"<<endl;
		ss << header;
